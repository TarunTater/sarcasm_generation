...
def prepare_sents(sents,max_len=47):
    x_batch = []
    y_batch = []
    seqls = []
    i = 0
    for sent in sents:
        ...
        for w in sent.split():
            ...
            i+=1
        array2 = np.array(array1)
        x_batch.append(array2)
        y_batch.append(0)
        seqls.append(len(array1))
    return [x_batch,y_batch,seqls]

    
def get_batch_reward_score(sents):
    scores = []
    x_batch,y_batch,seqls = prepare_sents(sents)
    out = sess.run([pred],feed_dict={batch_ph: x_batch,target_ph: y_batch,seq_len_ph: seqls,keep_prob_ph: 1.0})
    out = 1-out[0] 
    #print (out)
    return np.mean(out)
...
if __name__=="__main__":
    sents = ["i love being ignored"]*10
    print (get_batch_reward_score(sents))
    #while True:
    #    print ("Enter Sent")
    #    inp = input()
