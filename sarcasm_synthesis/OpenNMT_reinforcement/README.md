# Sarcasm Synthesis Module

This is an adaptaion of OpenNMT-py which also includes a Tensorflow based Sarcasm Scorer module for reward computation during reinforcement learning. 

Pleae check requirements.txt for installation requirement. You should also instal Tensorflow 1.7.0 (gpu/cpu).

For training the module, please run `cmd.sh`. The models should be saved in `model_dir`. Select appropriate models based on losses on validation data. 

For testing, follow the instructions below on how to decode using `translate.py`

If you want to train a model from scratch, please follow the instructions below.
