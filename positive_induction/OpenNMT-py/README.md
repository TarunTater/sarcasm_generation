# POSITIVE SENTIMENT INDUCTION MODULE

This is an adaptaion of OpenNMT-py. For training the module, please run `cmd.sh`. The models should be saved in `model_dir`. Select appropriate models based on losses on validation data. Note, the pre-processed data is inside `data` folder (the data has a reduced vocabulary size compared to the original one presented in the paper. However, it gives similar performance).
